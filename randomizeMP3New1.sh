...
whileDo () {
	while [ "$toPlay" != '0' ]; do
		...
		mplayer -xy 400 -geometry 1250:0 -playlist playNowMP3.tmp;
		#xplayer $(cat playNowMP3.tmp)
		cat toPlayMP3.txt | grep -v "$(cat playNowMP3.tmp | awk -F'/' '{print $NF}')" | sort -R > toPlayMP3.tmp;
		cp toPlayMP3.tmp toPlayMP3.txt;
...
touch playedMP3.tmp;
chmod 744 toPlayMP3.txt;
chmod 744 toPlayMP3.tmp;
toPlay=$(cat toPlayMP3.txt | wc -l);
...
if [ "$toPlay" -eq 0 ]; then
	...
	cat toPlayMP3.txt | sort -R > toPlayMP3.tmp;
	cp toPlayMP3.tmp toPlayMP3.txt;
fi
